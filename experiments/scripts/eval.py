...
def main(config, print_metrics=True):
    ...
    if config.test_path:
        ...
        test = pd.read_csv(config.test_path, usecols = ["smiles"], compression="xz", 
                          ).squeeze().values
    ...
    if config.train_path is not None:
        train = pd.read_csv(config.train_path, usecols = ["smiles"], compression="xz", 
                           ).squeeze().values
    ...
    gen = pd.read_csv(config.gen_path, usecols=["smiles"]).fillna("-").squeeze().values
    # gen.fillna("-", inplace=True)
    # gen=gen.squeeze().values
...
if __name__ == '__main__':
    ...
    metrics_path =''.join( os.path.splitext(config.gen_path)[:-1]) +'_metrics.csv'
