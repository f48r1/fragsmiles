...
class WordRNN(nn.Module):
    ...
    def __init__(self, vocabulary, config):
        ...
        if config.embedding_size <1:
            ...
            self.vocab_size = self.output_size = len(vocabulary)
            self.input_size =  config.embedding_size

        self.embedding_layer = nn.Embedding(self.vocab_size, self.input_size,
    ...
    def tensor2tokens(self, tensor):
        ids = tensor.tolist()
        return [self.vocabulary.id2char(id) for id in ids ]

    def sample(self, n_batch, max_length=40, temp=1):
        with torch.no_grad():
            ...
            for i in range(1, max_length + 1):
                ...
                # probabilities
                probs = [F.softmax(o / temp, dim=-1) for o in output]
            ...
            return [self.tensor2tokens(t) for t in new_smiles_list]
