...
## Add wordRNN to moses models to keep default pipeline of scripts
...
def get_generation_path(config, model):
    return os.path.join(
        ...
        model + config.experiment_suff + '_generated_T'+str(config.temp)+'.csv'
    )
...
def get_parser():
    ...
    parser.add_argument("--notation",
                        type=str, default='fragsmiles', required=False,
                        help="Molecular representation employed to feed model")
    parser.add_argument('--temp', type=float, default=1.0,
                        help='Temperature sampling for softmax')
    return parser
...
def train_model(config, model, train_path, test_path):
    ...
    trainer_config, unks = trainer_parser.parse_known_args(
         [model] + sys.argv[1:] + args
    )
    print("unknown arguments :",unks)
...
def sample_from_model(config, model):
    ...
    sampler_config, unk = sampler_parser.parse_known_args(
        [model] + sys.argv[1:] +
        ['--device', config.device,
...
def main(config):
    ...
    for model in models:
        ...
        if len(models) == 1:
            ...
            metrics_path = get_generation_path(config, model)
            metrics_path = ''.join(os.path.splitext(metrics_path)[:-1])+"_"+config.metrics
        else:
